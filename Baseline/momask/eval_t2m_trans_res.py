...
import utils.eval_t2m_moclip as eval_t2m_moclip
from utils.eval_wrapper_moclip import EvalWarperMoClip
...
if __name__ == '__main__':
    ...
    if use_moclip:
        eval_wrapper = EvalWarperMoClip(input_dim=dim_pose,model_path="/mnt/c/RBM/momask-codes/checkpoints/cma/clip_motion_align_epoch_16.pt")
    else:
    ...
    for file in os.listdir(model_dir):
        ...
        for i in range(repeat_time):
            with torch.no_grad():
                best_fid, best_div, Rprecision, best_matching, best_mm,best_mmd = \
                    eval_t2m_moclip.evaluation_mask_transformer_test_plus_res(eval_val_loader, vq_model, res_model, t2m_transformer,
        ...
        msg_final = f"\tFID: {np.mean(fid):.3f}, conf. {np.std(fid) * 1.96 / np.sqrt(repeat_time):.3f}\n" \
                    ...
                    f"\tCMMD:{np.mean(mmd):.3f}, conf.{np.std(mmd) * 1.96 / np.sqrt(repeat_time):.3f}\n\n" \
 \
