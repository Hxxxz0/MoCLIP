...
import mGPT.mo_clip as mo_clip
...
class TM2TMetrics(Metric):
    def __init__(self,
        ...
        self.add_state("gen_text_embeddings", default=[], dist_reduce_fx=None)
    ...
    def compute(self, sanity_flag):
        ...
        if self.text:
            ...
            # Compute r-precision
            all_texts_pred = torch.cat(self.gen_text_embeddings,axis=0).cpu()
            print(all_texts_pred.shape)
            all_texts_pred = all_texts_pred[shuffle_idx, :]
            assert count_seq > self.R_size
            top_k_mat = torch.zeros((self.top_k, ))
            for i in range(count_seq // self.R_size):
                # [bs=32, 1*256]
                group_texts = all_texts_pred[i * self.R_size:(i + 1) * self.R_size]
                # [bs=32, 1*256]
            ...
            all_texts_gt = torch.cat(
                self.gt_text_embeddings,
                axis=0).cpu()[
                             shuffle_idx,
                             :]
            assert count_seq > self.R_size
            ...
            for i in range(count_seq // self.R_size):
                # [bs=32, 1*256]
                group_texts = all_texts_gt[i * self.R_size:(i + 1) * self.R_size]
                # [bs=32, 1*256]
        ...
        metrics["MMD"] = mo_clip.compute_mmd(all_genmotions, all_gtmotions)
    ...
    def update(self,
        ...
        et,em = self.get_co_embeddings(gt_texts,feats_ref)
        gtmotion_embeddings = em
        ...
        et_pred,em_pred = self.get_co_embeddings(gt_texts,feats_rst)

        recmotion_embeddings = em_pred
        ...
        if self.text:
            ...
            self.gen_text_embeddings.append(et_pred)
            self.gt_text_embeddings.append(et)
    ...
    def get_co_embeddings(self, gt_texts: Tensor,feats: Tensor):
        return mo_clip.get_co_embeddings_2(gt_texts,feats) #caption,motion
